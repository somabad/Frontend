...
const GreetingCard = ({ adminName, loading, error }) => {
  if (loading) return <div>Loading...</div>;
  if (error) return <div>{error}</div>;

  return (
    <Card className='profile-box' style={{ height: '153px' }}>
      <CardBody>
        <Media className="d-flex justify-content-between align-items-center">
          {/* Left Side: Greeting */}
          <Media body>
            <div className='greeting-user'>
              <H4 attrH4={{ className: 'f-w-600' }}>Welcome, {adminName}</H4>
            </div>
          </Media>

          {/* Right Side: Cartoon and Clock */}
          <div className="d-flex align-items-center gap-3">
            <div className='cartoon'>
              <Image attrImage={{ src: CarToon, alt: 'vector women with laptop' }} style={{ width: '10px', height: 'auto' }} />
            </div>
            <div className='clockbox' style={{ width: '80px', height: '80px' }}>

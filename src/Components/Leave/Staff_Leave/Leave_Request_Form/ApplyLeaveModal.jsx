...
const translations = {
  en: {
    ...
    fileSizeNote:
      "Note: Please attach files no larger than 2 MB. Larger files will not be accepted.",
    cancel: "Cancel",
    submit: "Submit",
  ...
  ms: {
    ...
    fileSizeNote:
      "Nota: Sila lampirkan fail tidak melebihi 2 MB. Fail yang lebih besar tidak akan diterima.",
    cancel: "Cancel",
    submit: "Submit",
    ...
    errors: {
      noLeaveTypes:
        "Tiada jenis cuti tersedia. Anda telah menggunakan semua hari cuti yang diperuntukkan.",
    ...
    tooltip:
      "Jumlah hari akan dikira secara automatik berdasarkan tarikh mula dan tamat.",
    leaveTypes: {
      annual: "Cuti Tahunan",
      medical: "Cuti Sakit",
...
const ApplyLeaveModal = ({ isOpen, onClose, onSubmitted }) => {
    ...
    try {
      ...
      if (list && Array.isArray(list)) {
        const detail = list.find(
          (s) => String(s.staffId) === String(currentStaffId)
        );
        if (detail) {
      ...
      if (
        data &&
        (!staffName || !staffPosition || !staffDepartment || !totalDays)
      ) {
      ...
      if (balanceData) {
        ...
        const filteredLeaveTypes = leaveTypesToFilter.filter((leaveType) => {
          ...
          console.log(
            `Mapped "${leaveTypeName}" to balance key "${balanceKey}"`
          );
          ...
          if (!balance) {
            console.log(
              `No balance data for ${leaveTypeName}, keeping it available`
            );
            return true;
          }
          ...
          console.log(
            `${leaveTypeName} will be ${shouldShow ? "SHOWN" : "HIDDEN"}`
          );
          return shouldShow;
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const safeJobTakenOverBy = Array.isArray(jobTakenOverBy)
        ? jobTakenOverBy[0]
        : jobTakenOverBy;
      const safeAttachment = Array.isArray(attachment)
        ? attachment[0]
        : attachment;
            ...
            <div className="px-4 py-3 bg-light border-bottom">
              <div className="d-flex justify-content-between align-items-center position-relative">
                ...
                >
                  ...
                  <small
                    className={`text-center ${
                      currentStep >= 1 ? "fw-bold text-primary" : "text-muted"
                    }`}
                    style={{ fontSize: "11px" }}
                  >
                    {t.steps.step1}
                ...
                >
                  ...
                  <small
                    className={`text-center ${
                      currentStep >= 2 ? "fw-bold text-primary" : "text-muted"
                    }`}
                    style={{ fontSize: "11px" }}
                  >
                    {t.steps.step2}
                ...
                >
                  ...
                  <small
                    className={`text-center ${
                      currentStep >= 3 ? "fw-bold text-primary" : "text-muted"
                    }`}
                    style={{ fontSize: "11px" }}
                  >
                    {t.steps.step3}
                ...
                >
                  ...
                  <small
                    className={`text-center ${
                      currentStep >= 4 ? "fw-bold text-primary" : "text-muted"
                    }`}
                    style={{ fontSize: "11px" }}
                  >
                    {t.steps.step4}
                  ...
                  <div className="mb-3">
                    ...
                    >
                      ...
                      {availableLeaveTypes.map((type) => {
                        ...
                        const balanceKey =
                          leaveTypeMapping[originalName] || originalName;
